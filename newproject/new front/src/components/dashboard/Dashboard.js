...
const Dashboard = () => {
    ...
    useEffect(() => {
        fetch('/api/balance')
            .then(response => response.json())
            .then(data => {
                console.log('Fetched balance:', data); // Debugging log
                setBalance(data[0][0]); // Assuming the API returns balance as an object with a key 'balance'
            })
            .catch(error => console.error('Error:', error));
    }, []);
